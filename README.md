...
> ## ⚠️ 维护状态：本分支暂停维护
>
> **`main` 分支自 v1.4.0 起暂停积极维护。** 当前功能、已知限制与性能特征均已定型（见下文"速度限制"与"已知限制"）。
> 不再接受新功能 PR；如有严重 bug，请提 issue 说明复现步骤，视情况在 `test/*` 分支或新分支修复。
> 社区可 fork 继续演进。发布版（`trialfinder-1.4.0-*.zip`）已归档，供长期使用。
...
v1.4.0 是 **`main` 分支的最终维护版本**，包含自 v1.3.0 以来的：

- **B 流缓存（`--cache`）**：同种子断点快扫，中断恢复从断点继续；换聚类参数不失效
- **预测预筛 + 有界验证（`--predict-depth` / `--predict-gate`）**：手写轻量布局预测刷怪笼上界，流式剪枝，召回可调
- **默认全量高精度**：`--top-k` 隐藏（默认 0=关闭，所有粗聚类进 B 流）
- **GPU 内核预编译 cubin 打包**：无需 CUDA Toolkit
- **`--biome-check` 近似可用**：排除海洋/深海底（见"已知限制"）
- 30 万格半径实测 95.5 万候选 → 10 个密集结果，B 流 ~74 座/秒（14 核）

> 该版本已冻结：后续不再主动维护。发布 zip 均含预编译内核与运行时（GraalVM/标准 JRE 二选一）。
...
**核心事实：B 流（Jigsaw 拼接）是唯一真实瓶颈，且无法 GPU 化。** GPU 只加速"决定看哪里"（A 流枚举 + 密度预筛），不加速"去看"（B 流生成密室）。因此总耗时随**命中密室数**线性增长，与 GPU 快慢无关。

### 实测吞吐（参考机器：14 逻辑核 / RTX 4060）

| 阶段 | 吞吐/耗时 | 说明 |
|---|---|---|
| A 流枚举 + 密度预筛 | 毫秒~秒级 | GPU 直通；10k 半径 1058 候选 ~50ms |
| **B 流（Jigsaw 拼接）** | **~74 座/秒（14 线程）** | 每密室 ~31ms 单线程、14 线程 ~74 座/秒（实测） |
| 生成 10 万密室 | **~22 分钟** | 74 座/秒 × 10 万 |
| 首次 GPU 初始化 | ~0.2 s（热缓存）/ ~2 s（冷编译） | NVRTC+ptxas 一次性；已做磁盘缓存 |

### 估算公式

```
总耗时 ≈ 命中密室数 × (1 / B流吞吐) + 首次 GPU 初始化 + 枚举/聚类开销
```

- `--search-radius 10000`（~1058 候选、~几百命中）→ **秒级**
- `--search-radius 300000`（~95 万候选、~几千命中）→ **分钟级**
- `--search-radius 1000000+` / `--full-world`（数百万~数亿候选、数十万命中）→ **小时级**
- 调低 `--min-spawners` / `--min-structures` → 命中密室数上升 → 耗时近似线性上升

### 加速手段（按收益排序）

1. **`--cache`**：同种子重扫 / 调参重扫 / 中断续扫直接命中缓存，跳过 B 流（**最大收益**）
2. **`--predict-depth + --predict-gate`**：预测预筛剪掉低分簇（`--predict-gate 12` 召回 ~0.99）
3. **`--threads`**：调到接近物理核心数（B 流是 CPU 密集）
4. **`--check-top N`**：只对前 N 个结果做慢速精确检查（避免全量）
5. **`--prefilter-mode grid` + 无损密度预筛**：大半径下 95.5 万→2.58 万候选（37×），B 流从 50 分钟→~2 分钟
6. **`--top-k N`**：显式截断低密度聚类（更快但**可能漏结果**，默认关闭）

### 上限 / 硬限制

- **B 流单机极限**：14 核 ~74 座/秒；换更强 CPU / 更多线程可线性提升，但无法跨过"每密室一次 Jigsaw"这一常数
- **`--search-radius 10,000,000+`**：即使自动分片也需数小时~数天；建议 `--full-world`（逐分片内存有界）或减小半径
- **GPU 仅 Windows x86_64**：Linux/macOS 走 `--no-gpu` 纯 CPU（B 流不变，仅枚举变慢）
- **`--biome-check` 为近似**：温度/湿度精确，大陆度/侵蚀/深度/离岸度用确定性近似（`TerrainProvider` 样条表未移植）；只可靠排除海洋/深海底
