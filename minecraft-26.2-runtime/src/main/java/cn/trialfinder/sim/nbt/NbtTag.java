...
/**
 * 试炼密室模板加载所需的最小 NBT 标签类型。API 刻意对齐 Mojang 的 {@code net.minecraft.nbt}，
 * 使移植代码读起来自然。只读。
 */
public interface NbtTag {

    record Compound(Map<String, NbtTag> entries) implements NbtTag {
        ...
        public List getList(String key) {
            NbtTag tag = entries.get(key);
            return tag instanceof List list ? list : new List(java.util.List.of(), Type.END);
        }

        public Compound getCompound(String key) {
            NbtTag tag = entries.get(key);
            return tag instanceof Compound c ? c : new Compound(Map.of());
        }
    ...
    record List(java.util.List<NbtTag> elements, Type elementType) implements NbtTag {
        public int size() {
    ...
    enum Type {
        ...
        public static Type byId(int id) {
            ...
            throw new IllegalArgumentException("未知 NBT 类型 id: " + id);
        }
    }
}
